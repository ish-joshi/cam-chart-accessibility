...
class LibRadarLite(object):
    ...
    def createPackages(self):
        # Read the file of libs
        st = time.time()
        setof = set()
        with open(FILE_RULE, 'r') as file_rules:
            csv_rules_reader = csv.reader(file_rules, delimiter=',', quotechar='|')
            for row in csv_rules_reader:
                package, name, cat, web = row
                setof.add(package)
        self.packages = list(setof)
        ed = time.time()
        # print("List of charting packages constructed in : ", (ed - st))

    def add_package_toTrie(self, package):
        dexTrie.add(self.trie, package)
    ...
    def extract_dex(self):
        for dex_name in self.dex_names:
            ...
            for dex_class_def_obj in current_dex.dexClassDefList:
                ...
                ignore_package_keywords = {"Lcom/google/", "Landroid/", "Landroidx"}
                to_add = not any(map(lambda ignoring: class_name.startswith(ignoring), ignore_package_keywords))
    ...
    def compare(self):
        ...
        res = self.get_matching_libs()
        return res

    def get_matching_libs(self):
        out = []
        for package in self.packages:
            present, count = dexTrie.find_prefix(self.trie, package)
            if present:
                # print("RESULT: ", package, "   ", present, "  ", count)
                out.append({"package": package, "present": present, "count": count})
        return {"data": out, "count": len(out), "filename": self.apk_path}
